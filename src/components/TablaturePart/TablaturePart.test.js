...
describe('TablaturePart 1', () => {
	beforeAll(() => {
		useSelector.mockImplementation(() => ({}))
	});

	it('should not crash TablaturePart component', () => {
		const wrapper = shallow(<TablaturePart part={{}} />);
		const html_elem = wrapper.find('[data-test="part-render"]');
		expect(html_elem).toHaveLength(1);
	});
});

describe('TablaturePart 2', () => {
	beforeAll(() => {
		useSelector.mockImplementation(() => ({
			all_ids: ['1-1', '1-2'],
			by_id: {
	...
	it('should find Blocks components', () => {
		const wrapper = shallow(<TablaturePart part={{ id: 1 }} />);
